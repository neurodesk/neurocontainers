...
class RuntimeTests(unittest.TestCase):
    ...
    def capture(self, items, app="openreconacsrss", params=None):
    ...
    def test_slice_grappa_reference_geometry_caipi_and_replay(self):
        ...
        conn, directory = self.capture(items, "openreconslicegrappa")
        params = dict(
            inputdomain="kx-ky",
            preprocessed=True,
            kernelsize=3,
    ...
    def test_unique_sessions_and_missing_original_diagnostic(self):
        with patch.dict(os.environ, FIRE_POC_CAPTURE_ROOT=str(self.root)):
            for _ in range(2):
                poc.process(Connection([None]), {}, self.metadata, "openreconacsrss")
    ...
    def test_original_series_collision_is_rejected(self):
        ...
        with self.assertRaisesRegex(ValueError, "collides"):
            list(
                poc.reconstruct(
                    directory / "input.h5", "openreconacsrss", config, self.metadata
                )
            )
