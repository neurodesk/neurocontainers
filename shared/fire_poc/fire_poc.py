...
def parameters(config: Any) -> dict[str, Any]:
    if isinstance(config, str):
        try:
            ...
            return {}
    return config.get("parameters", {}) if isinstance(config, dict) else {}
...
def process(connection: Any, config: Any, metadata: Any, app: str) -> None:
    ...
    try:
        ...
        capture.close("captured")
        if params.get("mode", "capture") == "reconstruct":
            if isinstance(metadata, (str, bytes)):
...
def main() -> None:
    ...
    parser.add_argument(
        "--app", choices=["openreconacsrss", "openreconslicegrappa"], required=True
    )
    parser.add_argument(
