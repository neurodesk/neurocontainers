    ...
    if output_path:
        ...
        try:
            ...
            if hasattr(ismrmrd, 'Dataset'):
                ...
                # Only meaningful for 4D images
                n_repetitions = ismrmrd_data.shape[3] if ismrmrd_data.ndim > 3 else 1
                
                # Write each slice as a separate image
                print(f"💾 Writing {ismrmrd_data.shape[2]} slices and {n_repetitions} repetitions as separate images...")
                ...
                for rep_idx in range(n_repetitions):
                    for slice_idx in range(ismrmrd_data.shape[2]):
                        ...
                        slice_image.repetition = rep_idx
                        ...
                        if (slice_idx + 1) % 50 == 0 or slice_idx == ismrmrd_data.shape[2] - 1:
                            print(f"   Written {slice_idx + 1}/{ismrmrd_data.shape[2]} slices and {rep_idx + 1}/{n_repetitions} repetitions...")
...
def main():
    ...
    try:
        ...
        print(f"   Original volume shape: {ismrmrd_image.data.shape}")
        print(f"   Number of 2D slices saved: {ismrmrd_image.data.shape[2]}")
        print(f"   Number of repetitions saved: {ismrmrd_image.data.shape[3] if ismrmrd_image.data.ndim > 3 else 1}")
