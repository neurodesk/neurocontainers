    ...
    for acq, lin, phs in zip(group, lin, phs):
        if (lin < data.shape[1]) and (phs < data.shape[3]):
            ...
            if (rawHead[phs] is None) or (
                np.abs(acq.getHead().idx.kspace_encode_step_1 - acq.getHead().idx.user[5])
                < np.abs(rawHead[phs].idx.kspace_encode_step_1 - rawHead[phs].idx.user[5])
            ):
...
def process_image(images, connection, config, metadata):
    ...
    #    subprocess.run(['python', '/synCT/src/test_nifti.py'], check=True)
