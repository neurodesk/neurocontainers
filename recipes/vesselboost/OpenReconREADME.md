...
The main derived output is named `<source>_vesselboost_segmentation`, where `<source>` is the incoming source `SeriesDescription` or, when that is absent, the incoming `SequenceDescription`. If neither source name is available, the fallback name is `vesselboost_segmentation`. By default, OpenRecon returns restamped original MRA images first as `<source>_original`, then sends the VesselBoost segmentation. Optional sagittal and coronal reformat series are named `<source>_vesselboost_segmentation_sagittal` and `<source>_vesselboost_segmentation_coronal`.
...
| Keep original images | `sendoriginal` | boolean | `true` | Return restamped original MRA images first, before the `vesselboost_segmentation` output. Disable this to return only derived VesselBoost output series. |
...
Returned images are always emitted as new derived scanner series. Restamped originals are sent first with `Keep_image_geometry = 1` and a patched source `IceMiniHead`, so scanner-side processing stays attached to the original MRA geometry. VesselBoost segmentation and reformatted sagittal and coronal outputs are sent after the originals as explicit packed 3D MRD volumes with explicit MRD header geometry, no source `IceMiniHead`, `Keep_image_geometry = 0`, `partition_count = 1`, and the output `slice_count`.
