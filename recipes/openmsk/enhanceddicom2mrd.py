...
def CreateMrdHeader(dset):
    """Create an MRD header object.

    Compatible with ismrmrd-python >= 1.15, whose xsd classes are generated by
    xsdata and require all mandatory schema elements to be passed at
    construction (the old PyXB-style ``ismrmrdHeader()`` + attribute-assignment
    API no longer works). Optional sections are best-effort.
    """
    xsd = ismrmrd.xsd

    # --- field strength / resonance frequency ---
    try:
        b0 = float(dset.MagneticFieldStrength)
    except Exception:
        b0 = 3.0
    h1_hz = int(round(b0 * 4258e4)) or 127730000

    # --- geometry (Enhanced multi-frame vs classic) ---
    ...
    # DICOM PixelSpacing is [row(Y), col(X)].
    espace = xsd.encodingSpaceType(
        matrixSize=xsd.matrixSizeType(x=int(dset.Columns), y=int(dset.Rows), z=1),
        fieldOfView_mm=xsd.fieldOfViewMm(
            x=float(pixel_spacing[1]) * int(dset.Columns),
            y=float(pixel_spacing[0]) * int(dset.Rows),
            z=float(slice_thickness),
        ),
    )
    ...
    mrdHead = xsd.ismrmrdHeader(
        experimentalConditions=xsd.experimentalConditionsType(
            H1resonanceFrequency_Hz=h1_hz
        ),
        encoding=[enc],
        sequenceParameters=xsd.sequenceParametersType(),
    )

    # --- optional: measurement information (patientPosition is required by it) ---
    try:
        mi = xsd.measurementInformationType(patientPosition=dset.PatientPosition)
        try:
            mi.measurementID = dset.SeriesInstanceUID
        except Exception:
            pass
        try:
            mi.protocolName = dset.SeriesDescription
        except Exception:
            pass
        try:
            mi.frameOfReferenceUID = dset.FrameOfReferenceUID
        except Exception:
            pass
        mrdHead.measurementInformation = mi
    except Exception:
        pass

    # --- optional: acquisition system information ---
    try:
        asi = xsd.acquisitionSystemInformationType()
        try:
            asi.systemVendor = dset.Manufacturer
        except Exception:
            pass
        try:
            asi.systemModel = dset.ManufacturerModelName
        except Exception:
            pass
        try:
            asi.systemFieldStrength_T = b0
        except Exception:
            pass
        mrdHead.acquisitionSystemInformation = asi
    except Exception:
        pass
...
def main(args):
    ...
    print(ismrmrd.xsd.ToXML(mrdHead))
    ...
    # Write MRD Header
    mrdDset.write_xml_header(bytes(ismrmrd.xsd.ToXML(mrdHead), 'utf-8'))
