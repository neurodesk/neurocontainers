...
class ReleaseArtifact:
    ...
    filename: str
    release_file: Path
    #: False when the suite's version had no release file and the newest release
    #: for the recipe was used instead.
    exact: bool = True
...
def resolve_release_artifact(
    ...
    *,
    ...
    """Derive the artifact a recipe's release metadata points at.

    An explicit ``release_file`` wins. Otherwise ``releases/<recipe>/<version>.json``
    is preferred and the newest release for the recipe is used as a fallback.
    Returns ``None`` when the recipe has no usable release metadata.
    """
    ...
    if release_file is not None:
        candidate = Path(release_file)
        if not candidate.is_file():
            raise ReleaseArtifactError(f"Release metadata not found: {candidate}")
        resolved_version = resolved_version or candidate.stem
    else:
        recipe_releases = Path(releases_dir) / recipe
        candidate = recipe_releases / f"{resolved_version}.json" if resolved_version else None
        if candidate is None or not candidate.is_file():
            candidate, latest_version, _ = find_latest_release_file(recipe_releases)
            if candidate is None:
                # No release at all is normal for an unbuilt recipe. Release
                # files that exist but carry no usable build date are not.
                unusable = sorted(recipe_releases.glob("*.json"))
                if unusable:
                    listing = ", ".join(path.name for path in unusable)
                    raise ReleaseArtifactError(
                        f"No usable build date in any release metadata for {recipe}: "
                        f"{listing}. Release files are generated by the build "
                        "automation; regenerate rather than editing them by hand."
                    )
                return None
            exact = False
            resolved_version = latest_version or resolved_version or candidate.stem
    ...
    *,
    ...
    if artifact is None:
        if declared_is_usable:
            notes.append(
                f"No release metadata for {recipe}; using the 'container:' value {declared_text}."
            )
    ...
    if not artifact.exact:
        notes.append(
            f"No release metadata for {recipe} {version or '(unversioned)'}; falling back "
            f"to the newest release {artifact.version} from {artifact.release_file}."
        )
    ...
    return ContainerResolution(
        ...
        source="release-metadata" if artifact.exact else "release-metadata-latest",
        notes=tuple(notes) + search_notes,
        error=error,
    )
